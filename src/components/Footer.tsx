    ...
    <footer className="bg-slate-50 border-t border-slate-200 pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-6 md:px-8 grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
        <div className="md:col-span-2">
          <Link to="/" className="text-2xl font-black text-primary mb-8 tracking-tighter uppercase inline-block">De'Hydra</Link>
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-4">
            {[
              ...
              <li key={item.label}>
                <Link to={item.path} className="text-on-surface-variant hover:text-primary transition-colors text-sm font-medium">{item.label}</Link>
              </li>
            ))}
          </ul>
        </div>
            ...
            {[
              ...
              <li key={item.label}>
                {item.path.startsWith('/#') ? (
                  <a href={item.path} className="text-on-surface-variant hover:text-primary transition-colors text-sm font-medium">{item.label}</a>
                ) : (
                  <Link to={item.path} className="text-on-surface-variant hover:text-primary transition-colors text-sm font-medium">{item.label}</Link>
                )}
              </li>
